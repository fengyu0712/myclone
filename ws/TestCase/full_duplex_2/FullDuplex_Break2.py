...
result_path = os.path.join(Project_path.test_result_path, '打断测试-result%s.xls' % now)
...
def break_pre(sessionId):
    ...
    for i in range(3):
        if 0 < i < 3:
            sessionId = wakeup()
        result_list = main2(pattern_list, pre_path, timeout=5)
        try:
