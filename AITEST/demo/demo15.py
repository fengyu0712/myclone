...
for a in range(len(data_list)):
    ...
    for b in range(a, len(data_list)):
        if "m_aicloud_end" in data_list[a] and "m_aicloud_end" in data_list[b] and 9 > b - a > 0 and data_list[
                                                                                                     a:b] != []:
            list0 = data_list[a:b]
            for i in range(len(list0)):
                print(list0[i])
                time_list = list0[i].split(":", 1)
                if time_list[0] == "m_aicloud_end":
                    w.write_onlydata(x, 0, time_list[1])
                elif time_list[0] == "m_cloud_speech_trans_ack":
                    w.write_onlydata(x, 1, time_list[1])
                elif time_list[0] == "m_parse_speech_reply_to_tts":
                    w.write_onlydata(x, 2, time_list[1])
                elif time_list[0] == "io_open_default":
                    w.write_onlydata(x, 3, time_list[1])
                elif time_list[0] == "http_connect1":
                    w.write_onlydata(x, 4, time_list[1])
                elif time_list[0] == "http_read_header":
                    w.write_onlydata(x, 5, time_list[1])
                elif time_list[0] == "http_connect2":
                    w.write_onlydata(x, 6, time_list[1])
                elif time_list[0] == "SDL_RunAudio":
                    w.write_onlydata(x, 7, time_list[1])
                else:
                    print("error")
            x += 1
            break
