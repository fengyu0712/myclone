...
class TestAC:
    ...
    def test_AC_yb101(self, tdata):
        ...
        try:
            ...
            result = "P"
            Logger().info(u"【%s用例%s--%s】:测试通过！" % (product + deviceModel, test_id, test_content))
        finally:
            # 写入测试结果
            rd.push_onlydata(test_id, str(result))
            rd.push_onlydata(test_id, str(mid))

    @allure.feature(product_model[1])
    @pytest.mark.parametrize("tdata", Label_Case(product_model[1]))
    # @pytest.mark.flaky(reruns=3, reruns_delay=redis_case)#重试机制
    def test_AC_HB(self, tdata):
        Logger().debug("testdata:---" + str(tdata))
        deviceModel = product_model[1]
        data = Default_data(deviceModel)
        test_id = tdata[0]
        test_content = tdata[2]
        data['text'] = tdata[4]
